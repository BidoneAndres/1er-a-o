	...
	if(Empty(this->General)){
		...
		while(aux!=NULL){//RECORRE LA LISTA GENERAL...
			...
			if(aux->cityId==auxid){
				...
				printf("El dia mas calido de %s fue el %d del %d \n",text,dia,mes);//PRINTEO DEL DIA...
				auxid=aux->cityId;
				mayor_temperatura=-99;
			}
			aux=aux->next;
		}
	}		
	...
	if(Empty(this->Cordoba)){
		...
		while(aux!=NULL){//CALCULA LOS PROMEDIOS DE LAS CIUDADES COMO EN EL INCISO Nº3...
			...
			if(aux->cityId==auxid){
				...
				prom=suma/(float)cont;
				if(prom>mayor_promedio)//GUARDA EN mayor_promedio LOS DATOS DE LA CIUDAD...
				{
					mayor_promedio=prom;
					for( i=0;i<50;i++)
					{
